...
int CLUSTER_ext4_sync_file(struct file *file, loff_t start, loff_t end, int datasync)
{
	struct inode *inode = file->f_mapping->host;
	struct ext4_inode_info *ei = EXT4_I(inode);
	journal_t *journal = EXT4_SB(inode->i_sb)->s_journal;
	...
	if (journal->j_flags & JBD2_BARRIER &&
	    !jbd2_trans_will_send_data_barrier(journal, commit_tid))
		needs_barrier = true;
//printk("[CLUSTER] journal waiters %d\n", atomic_read(&journal->waiters));
	wait = CLUSTER_jbd2_wakeup_transaction(journal, commit_tid);
//printk("[CLUSTER] journal waiters after inc %d wait %d (1 wait 0 non-wait)\n", atomic_read(&journal->waiters), wait);
...
//printk("[CLUSTER] journal after dec %d\n", atomic_read(&journal->waiters));

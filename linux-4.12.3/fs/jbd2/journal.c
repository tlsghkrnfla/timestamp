...
EXPORT_SYMBOL(jbd2_journal_next_log_block);
...
// CLUSTER
struct buffer_head *
CLUSTER_jbd2_journal_get_descriptor_buffer(journal_t *journal, int type)
{
	//journal_t *journal = transaction->t_journal;
...
EXPORT_SYMBOL(CLUSTER_jbd2_journal_get_descriptor_buffer);

int CLUSTER_jbd2_journal_write_metadata_buffer(transaction_t *transaction,
...
repeat:
	...
	new_bh->b_size = bh_in->b_size;
	new_bh->b_bdev = journal->j_dev;
	new_bh->b_blocknr = (sector_t)(new_bh->b_end_io);
	new_bh->b_private = bh_in;
	set_buffer_mapped(new_bh);
	set_buffer_dirty(new_bh);

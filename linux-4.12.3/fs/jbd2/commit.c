...
void CLUSTER_jbd2_journal_commit_transaction(journal_t *);
...
{
	//if (journal->CLUSTER_journal) {
	//	CLUSTER_jbd2_journal_commit_transaction(journal);
	//	return;
	//}
					...
					__u32 crc32_sum, struct buffer_head *bh)
{
	...
	*cbh = NULL;
	...
	if (journal->j_flags & JBD2_BARRIER &&
	    !jbd2_has_feature_async_commit(journal))
		ret = CLUSTER_submit_bh(REQ_OP_WRITE,
			REQ_SYNC | REQ_PREFLUSH | REQ_FUA, bh);
		//ret = submit_bh(REQ_OP_WRITE,
		//	REQ_SYNC | REQ_PREFLUSH | REQ_FUA, bh);
	else
		ret = CLUSTER_submit_bh(REQ_OP_WRITE, REQ_SYNC, bh);
		//ret = submit_bh(REQ_OP_WRITE, REQ_SYNC, bh);
...
void CLUSTER_jbd2_journal_commit_transaction(journal_t *journal)
{
	...
	int csum_size = 0;
		...
		struct bio *bio, *last_bio, *first_bio = NULL;
...
//printk(KERN_ERR "[CLUSTER] journaling... tx->t_nr_buffers %d\n", commit_transaction->t_nr_buffers);
	...
	while (commit_transaction->t_buffers) {
		...
		//flags = jbd2_journal_write_metadata_buffer(commit_transaction, jh, &wbuf[bufs], blocknr);
...
start_journal_io:
			for (i = 0; i < bufs; i++) {
				...
				//submit_bh(REQ_OP_WRITE, REQ_SYNC, bh);
				bio = CLUSTER_make_bh_to_bio(REQ_OP_WRITE, REQ_SYNC, bh);
				if (!first_bio)
					first_bio = last_bio = bio;
				else {
...
//printk("[CLUSTER] journaling.. compare bufs %d total_bufs %d\n", bufs, total_bufs);
	...
	chain_data = &current->chain_data;
	chain_data->journal = journal;

	CLUSTER_submit_bio(REQ_OP_WRITE, REQ_SYNC, first_bio);
	//blk_finish_plug(&plug);

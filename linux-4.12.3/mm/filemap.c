...
// CLUSTER
ssize_t CLUSTER_generic_file_read(struct file *filp, loff_t *ppos,
									struct iov_iter *iter, ssize_t written)
{
	struct address_space *mapping = filp->f_mapping;
	struct inode *inode = mapping->host;
...
find_page:
		...
		page = find_get_page(mapping, index);
		if (!page) {
			page = CLUSTER_page_cache_sync_readahead(mapping, ra, filp,
									index, last_index - index);
			if (unlikely(page == NULL))
				goto no_cached_page;
		}
...
EXPORT_SYMBOL(CLUSTER_generic_perform_write);
...
int CLUSTER_filemap_fdatawait_range(struct address_space *mapping, loff_t start_byte,
			    loff_t end_byte)
{
	int ret, ret2;

	ret = CLUSTER__filemap_fdatawait_range(mapping, start_byte, end_byte);
	ret2 = filemap_check_errors(mapping);
	if (!ret)
		ret = ret2;
